$(document).ready(function () {
  document.getElementById("cart-wrapper").addEventListener("click", function () {
    ...
    var iteCount = JSON.parse(localStorage.getItem('ListItem'))
    console.log(iteCount)
    var Counting = Object.values(iteCount)
  ...
  $.get(link, function (res) {
    ...
    $("#btn-add-to-cart").click(function () {
      $("#cart-count").addClass('bigger')
      $("#btn-add-to-cart").css({ "background-color": "#4ba097" });
      setTimeout(function () {
        $('#cart-count').removeClass('bigger')
      ...
      //   resId : 1
      // }
      // ItemLister = JSON.parse(localStorage.getItem('ItemList'))
      // if (ItemLister == null) {
      //   localStorage.setItem('ItemList', JSON.stringify(res.id))
      // }
      // else {
      //   var data = ItemLister + res.id
